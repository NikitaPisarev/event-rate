...
@router.get("/new-events", response_description="List all available events")
...
async def update_event_status(event_id: str, status_update: EventStatusUpdate):
    result = await event_collection.update_one(
        {"_id": ObjectId(event_id)}, {"$set": status_update.model_dump()}
    )
...
@router.delete("/events/{event_id}", response_description="Delete an event")
